																	 ...
																	 void ** ppv )
{
	...
	CConCenComponent * pCCenCon = new CConCenComponent;
	...
	HRESULT hr = pCCenCon->QueryInterface(iid, ppv);
...
HRESULT CFactory::Factory::ReadFile()
{
	...
	ifstream in("d:\\projects\\sdwv3~1\\control.cfg", ios::in | ios::nocreate );
	
	if ( in.fail() )
	{
		MessageBox(0,"Cannot open configfile control.cfg",0,0);
		return E_FAIL;
	}
...
HRESULT CFactory::Factory::CreateLayers(CConCenComponent * pCCenCon)
{
	ISDWControlCentre * pCCen;
	ISDWPanel * pPan;
	ISDWWatch * pWatch;
	ISDWConnector * pCon;
	...
	hr = pPan->QueryInterface(IID_ISDWConnector, (void **) &pCon );
	...
	pCon->SDWSetUpperConnection(pWatch);
	pCon->Release();

	hr = pWatch->QueryInterface(IID_ISDWConnector, (void **) &pCon );
	...
	pCon->SDWSetUpperConnection(pCCen);
	pCon->SDWSetLowerConnection(pPan);
	pCon->Release();

	hr = pCCen->QueryInterface(IID_ISDWConnector, (void **) &pCon );
	...
	pCon->SDWSetLowerConnection(pWatch);
	pCon->SDWSetUpperConnection(pCCenCon);  
	pCon->Release();
	...
	pCon->SDWSetLowerConnection(pCCen);

	// This one costed almost 3 hours of real hard searching.
	pCon->Release();
