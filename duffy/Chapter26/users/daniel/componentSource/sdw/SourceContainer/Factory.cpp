																	 ...
																	 void ** ppv )
{
	...
	CSourceContainerComponent * pSrcCon = new CSourceContainerComponent;
	...
	HRESULT hr = pSrcCon->QueryInterface(iid, ppv);
...
HRESULT CFactory::Factory::ReadFile()
{
	...
	ifstream in("d:\\projects\\sdwv3~1\\source.cfg", ios::in | ios::nocreate );
	
	if ( in.fail() )
	{
		MessageBox(0,"Cannot open configfile source.cfg",0,0);
		return E_FAIL;
	}
...
HRESULT CFactory::Factory::CreateLayers(CSourceContainerComponent * pSrcCon)
{
	ISDWSource * pSrc;
	ISDWSensor * pSens;
	ISDWConverter * pConv;
	...
	hr = pSens->QueryInterface(IID_ISDWConnector, (void **) &pCon );
	...
	pCon->SDWSetUpperConnection(pConv);
	pCon->Release();

	hr = pConv->QueryInterface(IID_ISDWConnector, (void **) &pCon );
	...
	pCon->SDWSetUpperConnection(pSrc);
	pCon->SDWSetLowerConnection(pSens);
	pCon->Release();

	hr = pSrc->QueryInterface(IID_ISDWConnector, (void **) &pCon );
	...
	pCon->SDWSetLowerConnection(pConv);
	pCon->SDWSetUpperConnection(pSrcCon);  
	pCon->Release();
	...
	pCon->SDWSetLowerConnection(pSrc);
