...
//
// ** Explicit Euler ** NOT scheme for the 2-d heat equation.
// The objective is to get an idea of setting up fdm schemes.
//
// STUDENT PROJECT: copy the code from splitting implicit for the heat 
// equation so that PERFORMANCE BOOST
/////////////////////////////////////////////////////////////////
///
...
void DiscreteBCatTimeZero(const Vector<double, long>& xMesh, const Vector<double, long>& yMesh, 
						  TwoBasketOption& option, NumericMatrix<double, long>& Solution)
{
	...
	index = Solution.MinColumnIndex();
	// Bottom (Put option)
	for (i = Solution.MinRowIndex()+1; i <= Solution.MaxRowIndex(); i++)
	{
		Solution(i, index) = option.downOption.payoff(xMesh[i]);
	}
	...
	{
		Solution(index,j) = option.leftOption.payoff(yMesh[j]);
...
void DiscreteIC(const Vector<double, long>& xMesh, const Vector<double, long>& yMesh, 
				const TwoBasketOption& option, NumericMatrix<double, long>& Solution)
{
...
int main()
{
	...
	//
			...
			// Step sizes
			long NX = 160; long NY = 160; long NT = 7000;
	...
	// the discrete domain
	//
	NumericMatrix<double, long> MOld(xMesh.Size(), yMesh.Size(), 
										xMesh.MinIndex(), yMesh.MinIndex());
	...
	{
		repository = Tensor<double, long>(MOld.Rows(), MOld.Columns(), NT+1);
		storeData = true;
	}
...
//int uu; cin >> uu;
	...
	try 
	{
// Reader modify to suit needs
//		printTensorInExcel(repository, xMesh, yMesh, string("2d Heatexp"));
//		printMatrixInExcel(repository[2000], xMesh, yMesh, string("EXPE"));
	}
