...
template <class V>
Set<V>::Set(const std::set<V>& stlSet)
{ // Empty set
...
template <class V>
Set<V>::Set(const std::list<V>& con)
{ // From am STL container (list, vector)

	s = std::set<V>();
	
	// Create list iterator
	typename std::list<V>::const_iterator i;
...
{ // Insert another set
	...
	// Test this
	typename std::set<V>::iterator i = s.begin();
	std::insert_iterator<std::set<V> > insertiter(s, i);
...
{ // s1 and s2 have common elements?
	...
	typename std::set<V>::const_iterator iter;
