...
#ifndef ExcelMechanisms_CC
#define ExcelMechanisms_CC


#include <duffy/ExcelMechanisms.hh>
#include <duffy/DatasimException.hh>
#include <duffy/ExcelDriver.hh>
...
{ 
	...
	if (x.Size() != functionResult.Size())
	{
		...
		std::string tmp = std::string("Sizes of vectors are X, Y: ") + getString(x.Size()) + std::string(comma) + getString(functionResult.Size());

		throw DatasimException(std::string("Size mismatch of two vectors"), std::string("PrintOneExcel"),
			tmp);
	}


	std::cout << "Starting Excel\n";
	...
	// Workaround
	std::list<std::string> legendList; legendList.push_back(legend);
	std::list<Vector<double, long> > functionResultList; functionResultList.push_back(functionResult);
...
//	if (x.Size() != labels.size() || x.Size() != functionResult.size())
	...
	{
		std::string tmp = "Sizes of vectors are X, Y, Label: " + getString(x.Size()) + comma +
			getString(functionResult.size()) + comma + getString(labels.size());

		throw DatasimException(std::string("Size mismatch of vectors and labels"), std::string("PrintInExcel"),
			tmp);
	}
				 ...
				 const std::string& SheetName)
{
	...
	{
		std::string tmp = "Vectors X, Y, matrix M: " + getString(xarr.Size()) + comma + getString(yarr.Size())
			+ bracketL + getString(matrix.Rows()) + comma + getString(matrix.Columns()) + bracketR;

		throw DatasimException(std::string("Size mismatch of vector(s) and matrix"), std::string("PrintMatrixInExcel"),
			tmp);
	}
	...
	std::list<std::string> rowlabels = convertToString(xarr);
	std::list<std::string> columnlabels = convertToString(yarr);

	std::cout << "Staring Excel";
				 ...
				 const std::string& SheetName)
{
	...
	{
		std::string tmp = std::string("Vectors X, Y, matrix M: ") + getString(rowArr.Size()) + comma + getString(colArr.Size())
			 + bracketL + getString(matrix.Rows()) + comma + getString(matrix.Columns()) + bracketR;

		throw DatasimException(std::string("Size mismatch of vector(s) and matrix"), std::string("PrintMatrixChartsInExcel"),
			tmp);
	}
					...
					const std::string& legend)
{
	...
	{
		std::string tmp = "Sizes vectors X,Y1,Y2: " + getString(x.Size()) + comma + getString(y1.Size()) 
							+ comma + getString(y2.Size());

		throw DatasimException(string("Size mismatch of 3 vectors"), std::string("PrintDifferenceInExcel"),
			tmp);
	}
				 ...
				 const std::string& SheetName)
{
	...
	{
		std::string tmp1 = std::string(", x and y arrays: ") + bracketL + getString(xarr.Size()) + comma + getString(yarr.Size()) + bracketR;

		std::string tmp = "Incompatible matrices: [" + getString(matrix1.Rows()) + comma + getString(matrix1.Columns()) + bracketR + 
						bracketL + getString(matrix2.Rows()) + comma + getString(matrix2.Columns()) + bracketR;

		tmp += tmp1;

		throw DatasimException(std::string("Size mismatch of matrices"), std::string("PrintMatrixDifferenceInExcel"),tmp);
	}
	...
	if (xarr.Size() != m2.Rows() || yarr.Size() != m2.Columns())
	{
		...
		throw DatasimException(std::string("Size mismatch of vector(s) and matrix"), std::string("PrintMatrixDifferenceInExcel"),
			tmp);
	}
				...
				const std::string& SheetName, long freq)
{
	...
	{
		std::string tmp = std::string("Vectors X, Y, tensor M: ") + getString(xarr.Size()) + comma + getString(yarr.Size()) + 
						bracketL + getString(tensor.Rows()) + comma + getString(tensor.Columns()) + bracketR;

		throw DatasimException(std::string("Size mismatch of vector(s) and tensor"), std::string("PrintTensorInExcel"),
			tmp);
	}
