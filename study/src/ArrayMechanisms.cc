...
#ifndef ArrayMechanisms_CC
#define ArrayMechanisms_CC
...
{
	return std::sqrt( sumSquares(x) / V(x.Size()) );
}
...
template <class V, class I> SimplePropertySet<std::string, double> allAverages(const Vector<V,I>& x)
{
	...
	result.add(Property<std::string, double> ("SUM", sum(x)));
	result.add(Property<std::string, double> ("PRODUCT", product(x)));
	result.add(Property<std::string, double> ("SUMREC", sumReciprocals(x)));
	result.add(Property<std::string, double> ("SUMABSVAL", sumAbsoluteValues(x)));
	result.add(Property<std::string, double> ("MEAN", mean(x)));
	result.add(Property<std::string, double> ("GMEAN", geometricMean(x)));
	result.add(Property<std::string, double> ("HMEAN", harmonicMean(x)));
	result.add(Property<std::string, double> ("RMS", quadraticMean(x)));
	result.add(Property<std::string, double> ("SUMSQ", sumSquares(x)));
...
template <class V, class I> SimplePropertySet<std::string, double> allDispersions(const Vector<V,I>& x)
{
	...
	result.add(Property<std::string, double> ("MDEV", deviationFromMean(x)));
	result.add(Property<std::string, double> ("STD", standardDeviation(x)));
	result.add(Property<std::string, double> ("VARIANCE", variance(x)));
...
template <class V, class I> V mode(const Vector<V,I>& x)
{
	...
	// Exx. Code can be optimized

	std::map<V, I> elements;
	...
	typename std::map<V, I>::iterator i = elements.begin();
...
template <class V, class I> V lpNorm(const Vector<V,I>& x, const I& p)
{
	...
	for (I j = x.MinIndex(); j <= x.MaxIndex(); j++)
	{
		ans += std::pow(x[j], myPower);
	}

	return std::pow(ans, V(1.0)/ myPower);
}
...
template <class V, class I> SimplePropertySet<std::string, double> allNorms(const Vector<V,I>& x)
{
	...
	result.add(Property<std::string, double> ("l1", l1Norm(x)));
	result.add(Property<std::string, double> ("l2", ManyToManyRelationorm(x)));
	result.add(Property<std::string, double> ("linf", lInfinityNorm(x)));
...
{
	std::cout << "Tensor, Rows " << tensor.Rows() << ", Columns " << tensor.Columns() << 
			", Third Dimension " << tensor.sizeThird() << std::endl;
