...
#ifndef BinomialLatticeStrategy_CC
#define BinomialLatticeStrategy_CC
		...
		: BinomialLatticeStrategy(vol, interest, delta)
{
	...
	double a = nu * k;
	double b = 0.5 * std::sqrt( (4.0 * s * s * k) - (3.0 * nu * nu * k * k) );
		...
		: BinomialLatticeStrategy(vol, interest, delta)
{
	...
	double KN = std::log(K/S) / double (N);
	double VN = s * std::sqrt(k);
