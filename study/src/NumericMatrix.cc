...
#ifndef NumericMatrix_CC
#define NumericMatrix_CC
...
//	Matrix<V, I, S>::Row(row, val);
	...
	for (I c=this->MinColumnIndex(); c<=this->MaxColumnIndex(); c++) 
	{
...
{ // Unary minus
	...
	NumericMatrix<V, I, S> result(this->Rows(), this->Columns(), this->MinRowIndex(), this->MinColumnIndex());

	// Copy all elements negative
	for (I r=this->MinRowIndex(); r<=this->MaxRowIndex(); r++)
	{
		for (I c=this->MinColumnIndex(); c<=this->MaxColumnIndex(); c++) 
		{
...
{ // Add the elements
	...
	// Add all elements
	for (I r1=this->MinRowIndex(); r1<=this->MaxRowIndex(); r1++)
	{
		for (I c1=this->MinColumnIndex(); c1<=this->MaxColumnIndex(); c1++) 
  ...
  // using the InnerProduct function of Vector. This saves a couple of row and column copies.
	...
	for (I rr=result.MinRowIndex(); rr<=result.MaxRowIndex(); rr++)
	{
		for (I cr = result.MinColumnIndex(); cr <= result.MaxColumnIndex(); cr++) 
		{
			...
			for (I k = (*this).MinColumnIndex(); k <= (*this).MinColumnIndex(); k++)
			{
...
template <class V, class I, class S> Vector<V, I> NumericMatrix<V, I, S>::operator * (const Vector<V, I>& v) const
{
	...
	Vector<V, I> result(this->Rows(), v.MinIndex());
	...
	for (I i = this->MinRowIndex(); i <= this->MaxRowIndex(); i++)
	{
		...
		r = 0.0;
		for (I j = this->MinColumnIndex(); j<= this->MaxColumnIndex(); j++)
		{
				r += (*this)(i,j) * v[j];
		}
