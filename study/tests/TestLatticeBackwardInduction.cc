...
namespace BackwardInduction
{
		...
		double taverse(const Lattice<double, int, 2>& lattice)
		{
			...
			for (int n = lattice.MaxIndex() - 1; n >= lattice.MinIndex(); n--)
			{
				...
				for (int i = std::copy[j].MinIndex(); i <= std::copy[j].MaxIndex(); i++)
				{
			
		//			copy[n][i] = discounting * (p * copy[n+1][i+1] + (1.0-p) * copy[n+1][i]);
					lattice[n][i] = nodeValue(std::copy[n+1][i+1], std::copy[n+1][i])
...
int main()
{
	...
	BackwardInduction::createTerminalVector = mycreateTerminalVector(lattice);

	double upper = 
	double lower = 
	BackwardInduction::nodeValue = mynodeValue(uppper, lower, lattice);
