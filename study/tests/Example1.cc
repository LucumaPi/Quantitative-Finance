...
public:
	...
	virtual void print() const { std::cout << "I'm base\n"; }
	virtual double calculate(double d) const = 0;
	
	void doIt() { std::cout << "in base\n"; }
...
public:
	...
	virtual void print() const { std::cout << "I'm a D1\n"; }
...
public:
	...
	virtual void print() const { std::cout << "I'm a D2\n"; }
...
int main()
{
	...
	const std::type_info& myRef = typeid(d1);
	
	std::cout << "Human-readable name: " << myRef.name() << std::endl;
	...
	{
		std::cout << "Types are NOT the same\n";
	}


	Base* b = &d1;
	const std::type_info& myRef2 = typeid(*b);

	std::cout << "Human-readable name: " << myRef2.name() << std::endl;
	...
	for (int i = 0; i < size; i++)
	{
		...
		{
			std::cout << "We have a D1\n"; counterD1++;
		}

		if (typeid(*myArr[i]) == typeid(D2))
		{
			std::cout << "We have a D2\n"; counterD2++;
		}

	}

	// Print final counts
	std::cout << "Number of D1s: " << counterD1 << std::endl;
	std::cout << "Number of D2s: " << counterD2 << std::endl;
	...
	if (d1Cast == 0)
	{
		std::cout << "Cast not possible:\n";
	}
	else
	{ // This function gets called

		std::cout << "Cast is possible: ";
		d1Cast -> print();
	}
