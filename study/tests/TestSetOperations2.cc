...
template <class T> void printSet(const Set<std::pair<T, T> >& l, const std::string& name)
{  // Print the contents of a Set. Notice the presence of a constant iterator.
	
	std::cout << std::endl << name << ", size of set is " << l.Size() << "\n[ ";

	typename std::set<std::pair<T, T> >::const_iterator i;
...
int main()
{
	...
	if (s3.Subset(s1) == true)
		std::cout << "Yes, is subset OK\n";
	else
		std::cout << "Bah1\n";

	if (s1.Superset(s3) == true)
		std::cout << "Yes, is superset OK\n";
	else
		std::cout << "Bah1\n";

	if (s1.Intersects(s3) == true)
		std::cout << "Yes, we have intersection OK\n";
	else
		std::cout << "Bah1\n";

	Set<char> s4;
	if (s1.Intersects(s4) == false)
		std::cout << "Yes, we have NO intersection OK\n";
	else
		std::cout << "Bah1\n";
	...
	if (s5.Contains('A') == true)
		std::cout << "OK, contains A\n";
	else 
		std::cout << "Bah\n";

	if (s5.Contains('Z') == false)
		std::cout << "OK, contains NOT Z\n";
	else 
		std::cout << "Bah\n";
	...
	Set<std::pair <int, int> > S = s11.CartesianProduct(s12);
	printSet(S, "Pr");

	Set<std::pair <int, int> > S2 = s12.CartesianProduct(s11);
	printSet(S2, "Pr");
